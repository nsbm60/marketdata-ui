...
export default function ChartPanel({ selected }: ChartPanelProps) {
  ...
  // Build indicator settings for server-side computation
  const indicators = useMemo(() => ({
    atrPeriod: settings.atr.enabled ? settings.atr.period : undefined,
    rsiPeriod: settings.rsi.enabled ? settings.rsi.period : undefined,
    macdFast: settings.macd.enabled ? settings.macd.fast : undefined,
    macdSlow: settings.macd.enabled ? settings.macd.slow : undefined,
    macdSignal: settings.macd.enabled ? settings.macd.signal : undefined,
  }), [settings.atr.enabled, settings.atr.period, settings.rsi.enabled, settings.rsi.period,
       settings.macd.enabled, settings.macd.fast, settings.macd.slow, settings.macd.signal]);
  ...
  const {
    bars, liveCandle, loading, loadingMore, error, hasMore,
    atrData, rsiData, macdData,
    refresh, loadMore
  } = useChartData(
    activeSymbol,
    timeframe,
